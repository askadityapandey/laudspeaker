...
@Processor('{start.step}', {
  stalledInterval: process.env.START_STEP_PROCESSOR_STALLED_INTERVAL
    ? +process.env.START_STEP_PROCESSOR_STALLED_INTERVAL
    : 600000,
  removeOnComplete: {
    age: process.env.STEP_PROCESSOR_REMOVE_ON_COMPLETE_AGE
      ? +process.env.STEP_PROCESSOR_REMOVE_ON_COMPLETE_AGE
      : 0,
    count: process.env.START_STEP_PROCESSOR_REMOVE_ON_COMPLETE
      ? +process.env.START_STEP_PROCESSOR_REMOVE_ON_COMPLETE
      : 0,
  },
  metrics: {
    maxDataPoints: MetricsTime.ONE_WEEK,
  },
  concurrency: process.env.START_STEP_PROCESSOR_CONCURRENCY
    ? +process.env.START_STEP_PROCESSOR_CONCURRENCY
    : 1,
})
export class StartStepProcessor extends WorkerHost {
  constructor(
